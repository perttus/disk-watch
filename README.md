...
- If you are worried the incident caused abnormal SSD wear, check NVMe SMART counters such as `Percentage Used`, `Data Units Written`, `Media and Data Integrity Errors`, and `Error Information Log Entries` with `smartctl`; a large one-off write burst is not the same thing as confirmed drive wear. On macOS, a practical setup is `brew install smartmontools`, then `sudo smartctl -a /dev/disk0 | egrep 'Percentage Used|Data Units Written|Media and Data Integrity Errors|Error Information Log Entries'` for the internal SSD.
